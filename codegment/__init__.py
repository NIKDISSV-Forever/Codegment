...
CONFIG_FILENAME = str(Path(__file__).absolute().with_suffix('.ini'))
...
class CodeEditorApp(App):
    ...
    def open(self, fn: str):
        ...
        if lexer := pygments.lexers.find_lexer_class_for_filename(self.opened_file):
    ...
    def _suggest(self):
        ...
        for tp, words in suggestions.items():
            for word in words:
                items.append({'viewclass': 'TwoLineListItem', 'text': word, 'secondary_text': tp,
                              'on_release': self.paste(word)})
        if not items:
            return
    ...
    def _suggest_menu_with_items(self, items):
        return MDDropdownMenu(caller=self._code_inp, items=items, width_mult=4, opening_time=0, position='center')
